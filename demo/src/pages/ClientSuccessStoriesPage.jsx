...
const TestimonialsSection = () => {
  ...
  // Carousel navigation
  const goToPrev = () => setActiveIndex((prev) => (prev === 0 ? testimonials.length - 1 : prev - 1));
  const goToNext = () => setActiveIndex((prev) => (prev === testimonials.length - 1 ? 0 : prev + 1));
        ...
        <div className="relative" {...handlers}>
          <div className="relative bg-gray-800/50 border border-gray-700 rounded-2xl p-8 md:p-12">
            ...
            <button
              ...
              className="absolute left-4 top-1/2 -translate-y-1/2 bg-gray-700/60 hover:bg-teal-500/80 text-white rounded-full w-10 h-10 flex items-center justify-center z-20"
              style={{outline: 'none'}}
            >
            ...
            <button
              ...
              className="absolute right-4 top-1/2 -translate-y-1/2 bg-gray-700/60 hover:bg-teal-500/80 text-white rounded-full w-10 h-10 flex items-center justify-center z-20"
              style={{outline: 'none'}}
            >
          ...
          {/* Dots navigation (carousel indicators) */}
          <div className="flex justify-center mt-6 gap-2">
            {testimonials.map((_, index) => (
              <button
                key={index}
                onClick={() => setActiveIndex(index)}
                className={`w-3 h-3 rounded-full ${index === activeIndex ? 'bg-teal-500' : 'bg-gray-700'} transition-colors duration-200`}
